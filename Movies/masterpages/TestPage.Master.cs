...
namespace Movies.masterpages
{
    public partial class TestPage : System.Web.UI.MasterPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var model = ModelFactory.CreateModel<TestModel>(currentNode);
            model.Title = "Model Test Page";
            model.TimeStamp = DateTime.Now;

            ValidateModel(model);

            pageContent.Text = MustacheHelper.RenderMustacheTemplate(this, "testContent", model);
        }
        ...
        private void ValidateModel(TestModel model)
        {
            ...
            // uComponents
            AssertPropertyType(typeof(Movies.Models.Hyperlink), model.GetType().GetProperties().First(p => p.Name.Equals("UrlPickerField")));
            AssertPropertyType(typeof(List<Movies.Models.Hyperlink>), model.GetType().GetProperties().First(p => p.Name.Equals("MultiUrlPickerField")));
            ...
            AssertPropertyType(typeof(Movies.Models.Hyperlink), model.MoviesGrid.Items[0].GetType().GetProperties().First(p => p.Name.Equals("UrlPickerField")));
